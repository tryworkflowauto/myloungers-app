  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [tesisRes, sezonRes, gruplarRes, sezlonglarRes, rezRes, yorumRes, siparisRes, bekleyenSiparisRes, yaklasanRezRes, yaklasanBakiyeRes, haftalikGelirRes, gunlukGelirRes, aktifMusteriRes, cevaplanmayanYorumRes] = await Promise.all([
        ...
        supabase.from("rezervasyonlar").select("id, baslangic_tarih, bitis_tarih, kisi_sayisi, kullanici_id, kullanicilar(ad)").eq("tesis_id", tesisId).not("pgtranid", "is", null).gte("bitis_tarih", today).order("baslangic_tarih", { ascending: true }).limit(10),
