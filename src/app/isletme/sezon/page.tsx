...
// â”€â”€ Helpers for mapping from DB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function sezonBadge(bas: string, bit: string, aktif: boolean): { badge: string; badgeBg: string; badgeColor: string; borderColor?: string; rowBg?: string; opacity?: number } {
  ...
  if (aktif && nowStr >= bas && nowStr <= bit) return { badge: "Åžu an aktif", badgeBg: "#DBEAFE", badgeColor: "#1E40AF", borderColor: BLUE, rowBg: "#EFF6FF" };
  if (bitDate > now && basDate > now) return { badge: "YaklaÅŸÄ±yor", badgeBg: "#FFEDD5", badgeColor: "#C2410C" };
...
const GRUP_COLORS: Record<string, { bg: string; text: string }> = {
  "â­ Gold":    { bg: "#F5F3FF", text: PURPLE   },
  "ðŸ”¥ VIP":    { bg: "#FFEDD5", text: "#C2410C" },
  "âš“ Ä°skele": { bg: "#FEF3C7", text: "#92400E" },
  "ðŸŒŠ Silver": { bg: "#DBEAFE", text: "#1E40AF" },
};
const GRUPLAR_LIST = ["â­ Gold", "ðŸ”¥ VIP", "âš“ Ä°skele", "ðŸŒŠ Silver"];
const SEZON_RENKLERI = [BLUE, ORANGE, TEAL, PURPLE, GREEN, GRAY300];
const DEFAULT_FIYAT = 1000;

// â”€â”€ Helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const MONTHS_TR = ["Oca","Åžub","Mar","Nis","May","Haz","Tem","AÄŸu","Eyl","Eki","Kas","Ara"];
function fmtTarih(bas: string, bit: string) {
  const b = new Date(bas + "T00:00:00"), e = new Date(bit + "T00:00:00");
  return `${b.getDate()} ${MONTHS_TR[b.getMonth()]} â€” ${e.getDate()} ${MONTHS_TR[e.getMonth()]} ${e.getFullYear()}`;
}
function chipOf(d: KampanyaDurum) {
  return { aktif: "â— Aktif", planli: "â—· PlanlandÄ±", tamamlandi: "âœ“ TamamlandÄ±", durduruldu: "â¸ Durduruldu" }[d];
}
function kalanGun(bit: string) {
  const diff = Math.round((new Date(bit + "T00:00:00").getTime() - new Date("2026-03-10").getTime()) / 86400000);
  return diff > 0 ? `${diff} gÃ¼n` : null;
}

const emptySezonForm = { name: "", bas: "", bit: "", renk: BLUE };
const emptyKampForm  = { name: "", bas: "2026-03-11", bit: "2026-03-31", tip: "oran" as "oran"|"sabit", indirimOran: 20, sabitFiyatlar: {} as Record<string,number>, gruplar: ["â­ Gold","ðŸ”¥ VIP","ðŸŒŠ Silver"], musteriGoster: true };
...
export default function IsletmeSezonPage() {
  ...
  const [genelAyarlar, setGA] = useState({ minRezSure: 2, erkenIndirim: 10, grupBonus: 15, sonDakikaToggle: true, sonDakikaPct: 20, iptalPolitika: "24 saat Ã¶ncesine kadar tam iade", ozelSaat: 6 });
  ...
  async function fetchKampanyalar() {
    ...
    try {
      ...
      if (error || !data) {
        if (error) console.error("Kampanyalar Ã§ekilemedi:", error);
        setKampanyalar([]);
        return;
      }
  ...
  // Genel Ayarlar: tesisler tablosundan yÃ¼kle
  useEffect(() => {
    if (!tesisId) return;
    supabase
      .from("tesisler")
      ...
      .then(({ data, error }) => {
        ...
        const isOzel = iptal && !["24 saat Ã¶ncesine kadar tam iade", "48 saat Ã¶ncesine kadar tam iade", "Ä°ade yok", "%50 iade"].includes(iptal);
  ...
  // â”€â”€ Sezon actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function saveSezon() {
    ...
    if (editSezon) {
      ...
      if (error) { showToast("âŒ Sezon gÃ¼ncellenemedi"); return; }
      setSezonlar(p => p.map(s => s.id === editSezon.id ? { ...s, name, bas, bit, dot: renk } : s));
      showToast(`âœ… "${name}" sezonu gÃ¼ncellendi`);
    } else {
      ...
      if (error) {
        ...
        showToast(`âŒ Sezon eklenemedi: ${error.message ?? JSON.stringify(error)}`);
        return;
      }
      if (!data) { showToast("âŒ Sezon eklenemedi"); return; }
      ...
      showToast(`âœ… "${name}" sezonu eklendi`);
    }
    setSezonModal(false);
  }
  ...
  function anlikFiyat(f: FiyatRow) {
    ...
    return `â‚º${v.toLocaleString("tr")}`;
  }
  async function kaydetDegisiklikler() {
    ...
    showToast("âœ… DeÄŸiÅŸiklikler kaydedildi!");
  }

  async function kaydetGenelAyarlar() {
    if (!tesisId) {
      showToast("âŒ KayÄ±t baÅŸarÄ±sÄ±z");
      return;
    }
    const iptal_politikasi =
      genelAyarlar.iptalPolitika === "ozel"
        ? `${genelAyarlar.ozelSaat} saat Ã¶ncesine kadar tam iade`
  ...
  // â”€â”€ Kampanya actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function saveKampanya() {
    ...
    const payload = {
      tesis_id: tesisId,
      ad: kampForm.name,
      aciklama: "", // mevcut formda ayrÄ± aÃ§Ä±klama alanÄ± yok
    ...
    showToast(`âœ… "${kampForm.name}" kampanyasÄ± oluÅŸturuldu`);
    setKampModal(false);
    fetchKampanyalar();
  }
  ...
  function kopyala(k: Kampanya) {
    ...
    showToast(`ðŸ”„ "${k.name}" kopyalandÄ± â€” taslak oluÅŸturuldu`);
  }
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: GRAY100, color: GRAY800, display: "flex", flexDirection: "column", minHeight: "100%" }}>
      ...
      <header style={{ background: "white", borderBottom: `1px solid ${GRAY200}`, padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        <div>
          ...
          <span style={{ fontSize: 11, color: GRAY400 }}>2026 Sezonu â€¢ Åžu an: Erken Sezon</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={openKampEkle} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: ORANGE, color: "white", cursor: "pointer" }}>ðŸŽ¯ Kampanya OluÅŸtur</button>
          <button onClick={kaydetDegisiklikler} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: TEAL, color: "white", cursor: "pointer" }}>ðŸ’¾ DeÄŸiÅŸiklikleri Kaydet</button>
        </div>
      </header>
      ...
      <div style={{ padding: 24, flex: 1 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ background: "white", borderRadius: 14, border: `1px solid ${GRAY200}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${GRAY100}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: NAVY }}>ðŸ“… Sezon TanÄ±mlarÄ±</h3>
              <button onClick={openSezonEkle} style={{ padding: "5px 10px", fontSize: 11, fontWeight: 600, borderRadius: 8, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>âž• Sezon Ekle</button>
            </div>
            <div style={{ padding: 20 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: GRAY400, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>2026 YÄ±l GÃ¶rÃ¼nÃ¼mÃ¼</div>
              <div style={{ background: GRAY100, borderRadius: 12, height: 36, position: "relative", overflow: "hidden", marginBottom: 6 }}>
                <div style={{ position: "absolute", left: 0,       top: 0, bottom: 0, width: "16.5%", background: GRAY300, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "white" }}>KapalÄ±</div>
                ...
                <div style={{ position: "absolute", left: "41.5%", top: 0, bottom: 0, width: "25%",   background: ORANGE,  display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "white" }}>YÃ¼ksek</div>
                ...
                <div style={{ position: "absolute", left: "83%",   top: 0, bottom: 0, width: "17%",   background: GRAY300, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "white" }}>KapalÄ±</div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 24 }}>
                {sezonlar.map((s) => (
                  <div key={s.id} style={{ border: `1.5px solid ${s.borderColor ?? GRAY200}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, background: s.rowBg ?? "transparent", opacity: s.opacity ?? 1 }}>
                    ...
                    <button onClick={() => openSezonDuzenle(s)} style={{ padding: "5px 10px", fontSize: 11, fontWeight: 600, borderRadius: 8, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>âœï¸</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: "white", borderRadius: 14, border: `1px solid ${GRAY200}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${GRAY100}` }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: NAVY }}>âš™ï¸ Genel Ayarlar</h3>
            </div>
            <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
              {/* Minimum Rezervasyon SÃ¼resi */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: GRAY50, borderRadius: 10 }}>
                <div><div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>Minimum Rezervasyon SÃ¼resi</div><div style={{ fontSize: 11, color: GRAY400 }}>MÃ¼ÅŸteri en az kaÃ§ saat Ã¶nceden rezervasyon yapabilir</div></div>
              ...
              {/* Erken Rezervasyon Ä°ndirimi */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: GRAY50, borderRadius: 10 }}>
                <div><div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>Erken Rezervasyon Ä°ndirimi</div><div style={{ fontSize: 11, color: GRAY400 }}>3+ gÃ¼n Ã¶nceden yapÄ±lan rezervasyonlara indirim</div></div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: GRAY50, borderRadius: 10 }}>
                <div><div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>Grup Rezervasyon Bonusu</div><div style={{ fontSize: 11, color: GRAY400 }}>4+ ÅŸezlong aynÄ± anda rezerve edilirse</div></div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {/* Son Dakika Ä°ndirimi */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: GRAY50, borderRadius: 10 }}>
                <div><div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>Son Dakika Ä°ndirimi</div><div style={{ fontSize: 11, color: GRAY400 }}>GÃ¼n iÃ§i boÅŸ kalan ÅŸezlonglara otomatik indirim</div></div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ padding: "12px 14px", background: GRAY50, borderRadius: 10 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div><div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>Ä°ptal PolitikasÄ±</div><div style={{ fontSize: 11, color: GRAY400 }}>Rezervasyon iptalinde iade sÃ¼resi</div></div>
                  <select value={genelAyarlar.iptalPolitika} onChange={(e) => setGAf("iptalPolitika", e.target.value)} style={{ padding: "7px 10px", border: `1.5px solid ${GRAY200}`, borderRadius: 8, fontSize: 12 }}>
                    <option>24 saat Ã¶ncesine kadar tam iade</option>
                    <option>48 saat Ã¶ncesine kadar tam iade</option>
                    <option>Ä°ade yok</option>
                    <option>%50 iade</option>
                    <option value="ozel">âœï¸ Ã–zel SÃ¼re Belirle</option>
                  </select>
                </div>
                {genelAyarlar.iptalPolitika === "ozel" && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${GRAY200}` }}>
                    <span style={{ fontSize: 12, color: GRAY600, whiteSpace: "nowrap" }}>Ä°ade sÃ¼resi:</span>
                    <input
                      type="number"
                    ...
                    />
                    <span style={{ fontSize: 12, color: GRAY600, whiteSpace: "nowrap" }}>saat Ã¶ncesine kadar tam iade</span>
                    <span style={{ marginLeft: "auto", fontSize: 10, color: GRAY400 }}>1 â€“ 168 saat arasÄ±</span>
                  </div>
                )}
              </div>
              <button onClick={kaydetGenelAyarlar} style={{ padding: "9px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: TEAL, color: "white", cursor: "pointer", alignSelf: "flex-end" }}>ðŸ’¾ Kaydet</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "white", borderRadius: 14, border: `1px solid ${GRAY200}`, overflow: "hidden", marginBottom: 16 }}>
          <div style={{ padding: "16px 20px", borderBottom: `1px solid ${GRAY100}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: NAVY }}>ðŸ’° Grup FiyatlarÄ±</h3>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 11, color: GRAY400 }}>Sezon seÃ§:</span>
              <select value={seciliSezon} onChange={(e) => setSeciliSezon(e.target.value)} style={{ padding: "6px 10px", border: `1px solid ${GRAY200}`, borderRadius: 8, fontSize: 12 }}>
                <option value="erken">ðŸ”µ Erken Sezon</option>
                <option value="yuksek">ðŸŸ  YÃ¼ksek Sezon</option>
                <option value="normal">ðŸŸ¢ Normal Sezon</option>
              </select>
              <button onClick={kaydetDegisiklikler} style={{ padding: "6px 12px", fontSize: 11, fontWeight: 600, borderRadius: 8, border: "none", background: TEAL, color: "white", cursor: "pointer" }}>ðŸ’¾ Kaydet</button>
            </div>
          </div>
          <div style={{ padding: "0 8px", overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Grup","Erken Sezon","YÃ¼ksek Sezon","Normal Sezon","Min. SÃ¼re","AnlÄ±k Fiyat"].map((h) => (
                ...
                {fiyatlar.map((g) => (
                  <tr key={g.id} style={{ transition: "background 0.15s" }}>
                    ...
                    <td style={{ padding: "12px 14px", borderBottom: `1px solid ${GRAY100}` }}>
                      ...
                      <span style={{ fontSize: 11, color: GRAY400 }}> â‚º</span>
                    </td>
                    <td style={{ padding: "12px 14px", borderBottom: `1px solid ${GRAY100}` }}>
                    ...
                    <td style={{ padding: "12px 14px", borderBottom: `1px solid ${GRAY100}` }}>
                      ...
                      <span style={{ fontSize: 11, color: GRAY400 }}> gÃ¼n</span>
                    </td>
        ...
        <div style={{ background: "white", borderRadius: 14, border: `1px solid ${GRAY200}`, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: `1px solid ${GRAY100}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: NAVY }}>ðŸŽ¯ Kampanyalar</h3>
            <button onClick={openKampEkle} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: ORANGE, color: "white", cursor: "pointer" }}>âž• Kampanya OluÅŸtur</button>
          </div>
          <div style={{ padding: 20 }}>
            {kampanyaLoading ? (
              <div style={{ padding: 20, textAlign: "center", fontSize: 12, color: GRAY400 }}>Kampanyalar yÃ¼kleniyor...</div>
      ...
      {/* â”€â”€ SEZON MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {sezonModal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setSezonModal(false)}>
          <div style={modalBox(440)} onClick={(e) => e.stopPropagation()}>
            <div style={{ padding: "20px 24px", borderBottom: `1px solid ${GRAY200}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: NAVY }}>{editSezon ? "âœï¸ Sezonu DÃ¼zenle" : "ðŸ“… Sezon Ekle"}</h3>
              <button onClick={() => setSezonModal(false)} style={{ width: 30, height: 30, border: "none", background: GRAY100, borderRadius: 8, cursor: "pointer", fontSize: 14 }}>âœ•</button>
            </div>
            <div style={{ padding: "20px 24px" }}>
              <div style={{ marginBottom: 14 }}>
                <label style={labelCls}>Sezon AdÄ± *</label>
                <input type="text" value={sezonForm.name} onChange={(e) => setSezonForm(p => ({ ...p, name: e.target.value }))} placeholder="Ã¶rn: Erken Sezon, YÃ¼ksek Sezon..." style={inputCls} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
                <div>
                  <label style={labelCls}>BaÅŸlangÄ±Ã§ Tarihi *</label>
                  <input type="date" value={sezonForm.bas} onChange={(e) => setSezonForm(p => ({ ...p, bas: e.target.value }))} style={inputCls} />
                </div>
                <div>
                  <label style={labelCls}>BitiÅŸ Tarihi *</label>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: GRAY50, borderRadius: 10 }}>
                  ...
                  <span style={{ fontSize: 12, color: GRAY600 }}>SeÃ§ili renk Ã¶nizlemesi</span>
                </div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", padding: "16px 24px", borderTop: `1px solid ${GRAY200}` }}>
              <button onClick={() => setSezonModal(false)} style={{ padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>Ä°ptal</button>
              <button onClick={saveSezon} disabled={!sezonForm.name || !sezonForm.bas || !sezonForm.bit} style={{ padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: !sezonForm.name || !sezonForm.bas || !sezonForm.bit ? GRAY200 : TEAL, color: !sezonForm.name || !sezonForm.bas || !sezonForm.bit ? GRAY400 : "white", cursor: !sezonForm.name || !sezonForm.bas || !sezonForm.bit ? "not-allowed" : "pointer" }}>âœ… {editSezon ? "GÃ¼ncelle" : "Sezon Ekle"}</button>
            </div>
          </div>
        </div>
      )}

      {/* â”€â”€ KAMPANYA MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {kampModal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setKampModal(false)}>
          <div style={modalBox(540)} onClick={(e) => e.stopPropagation()}>
            <div style={{ padding: "20px 24px", borderBottom: `1px solid ${GRAY200}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: NAVY }}>{editKamp ? "âœï¸ Kampanya DÃ¼zenle" : "ðŸŽ¯ Yeni Kampanya OluÅŸtur"}</h3>
              <button onClick={() => setKampModal(false)} style={{ width: 30, height: 30, border: "none", background: GRAY100, borderRadius: 8, cursor: "pointer", fontSize: 14 }}>âœ•</button>
            </div>
            <div style={{ padding: "20px 24px" }}>
              {/* Ä°sim */}
              <div style={{ marginBottom: 14 }}>
                <label style={labelCls}>Kampanya AdÄ± <span style={{ color: GRAY400, fontWeight: 400 }}>(MÃ¼ÅŸteride gÃ¶rÃ¼nÃ¼r)</span></label>
                <input type="text" value={kampForm.name} onChange={(e) => setKampForm(p => ({ ...p, name: e.target.value }))} placeholder="Ã¶rn: Bahar Ä°ndirimi, Yaz AÃ§Ä±lÄ±ÅŸ..." style={inputCls} />
              </div>
              {/* Tarihler */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
                <div>
              ...
              {/* Ä°ndirim tipi */}
              <div style={{ marginBottom: 14 }}>
                <label style={labelCls}>Ä°ndirim Tipi</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {([
                    { key: "oran", icon: "%", name: "Oran Ä°ndirimi", desc: "Mevcut fiyattan % indirim" },
                    { key: "sabit", icon: "â‚º", name: "Sabit Fiyat", desc: "Her grup iÃ§in sabit fiyat gir" },
                  ] as const).map((op) => (
              ...
              {kampForm.tip === "oran" && (
                <div style={{ marginBottom: 14 }}>
                  <label style={labelCls}>Ä°ndirim OranÄ±</label>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
                  {GRUPLAR_LIST.map((gr) => (
                    <div key={gr}>
                      ...
                      <input type="number" value={kampForm.sabitFiyatlar[gr] ?? ""} onChange={(e) => setKampForm(p => ({ ...p, sabitFiyatlar: { ...p.sabitFiyatlar, [gr]: Number(e.target.value) } }))} placeholder="â‚º" style={inputCls} />
                    </div>
                  ))}
                </div>
              )}
              ...
              {/* MÃ¼ÅŸteri gÃ¶ster */}
              <div style={{ marginBottom: 14 }}>
                <label style={labelCls}>MÃ¼ÅŸteri TarafÄ±nda GÃ¶ster</label>
                <div style={{ display: "flex", alignItems: "center", gap: 10, background: GRAY50, padding: 12, borderRadius: 10 }}>
                  ...
                  <span style={{ fontSize: 12, color: GRAY600 }}>Rezervasyon sayfasÄ±nda kampanya adÄ± ve indirimi gÃ¶ster</span>
                </div>
              </div>
              {/* Ã–nizleme */}
              {kampForm.tip === "oran" && kampForm.indirimOran > 0 && kampForm.gruplar.length > 0 && (
                <div style={{ background: GRAY50, borderRadius: 10, padding: 14, border: `1px solid ${GRAY200}` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: GRAY400, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>Fiyat Ã–nizleme (%{kampForm.indirimOran} indirim)</div>
                  {kampForm.gruplar.slice(0, 3).map((gr) => {
                    ...
                    return base > 0 ? (
                      <div key={gr} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 6 }}>
                        ...
                        <span><s style={{ color: GRAY400 }}>â‚º{base.toLocaleString("tr")}</s> â†’ <strong style={{ color: ORANGE }}>â‚º{disc.toLocaleString("tr")}</strong></span>
                      </div>
                    ) : null;
                  })}
                </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", padding: "16px 24px", borderTop: `1px solid ${GRAY200}` }}>
              <button onClick={() => setKampModal(false)} style={{ padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>Ä°ptal</button>
              <button onClick={() => { saveKampanya(); }} disabled={!kampForm.name} style={{ padding: "8px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "none", background: !kampForm.name ? GRAY200 : ORANGE, color: !kampForm.name ? GRAY400 : "white", cursor: !kampForm.name ? "not-allowed" : "pointer" }}>
                {editKamp ? "ðŸ’¾ GÃ¼ncelle" : "ðŸš€ KampanyayÄ± Kaydet"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* â”€â”€ DURDUR ONAY MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {durdurModal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setDurdurModal(null)}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 380, maxWidth: "95vw", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", textAlign: "center" }}>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 8 }}>KampanyayÄ± Durdur</h3>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button onClick={() => setDurdurModal(null)} style={{ padding: "9px 22px", borderRadius: 8, fontSize: 13, fontWeight: 600, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>VazgeÃ§</button>
      ...
      {/* â”€â”€ BAÅžLAT ONAY MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {baslatModal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setBaslatModal(null)}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 380, maxWidth: "95vw", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", textAlign: "center" }}>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 8 }}>KampanyayÄ± BaÅŸlat</h3>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button onClick={() => setBaslatModal(null)} style={{ padding: "9px 22px", borderRadius: 8, fontSize: 13, fontWeight: 600, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>VazgeÃ§</button>
              <button onClick={() => { setKampanyalar(p => p.map(k => k.id === baslatModal.id ? { ...k, durum: "aktif" } : k)); showToast(`â–¶ "${baslatModal.name}" baÅŸlatÄ±ldÄ±`); setBaslatModal(null); }} style={{ padding: "9px 22px", borderRadius: 8, fontSize: 13, fontWeight: 600, border: "none", background: GREEN, color: "white", cursor: "pointer" }}>â–¶ Åžimdi BaÅŸlat</button>
            </div>
          </div>
        </div>
      )}

      {/* â”€â”€ SÄ°L ONAY MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {silModal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setSilModal(null)}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 380, maxWidth: "95vw", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", textAlign: "center" }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>ðŸ—‘ï¸</div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 8 }}>KampanyayÄ± Sil</h3>
            <p style={{ fontSize: 13, color: GRAY600, marginBottom: 4 }}>Bu iÅŸlem geri alÄ±namaz.</p>
            <p style={{ fontSize: 15, fontWeight: 700, color: NAVY, marginBottom: 20 }}>{silModal.name}</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button onClick={() => setSilModal(null)} style={{ padding: "9px 22px", borderRadius: 8, fontSize: 13, fontWeight: 600, border: `1px solid ${GRAY200}`, background: GRAY100, color: GRAY800, cursor: "pointer" }}>VazgeÃ§</button>
